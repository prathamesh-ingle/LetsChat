...
const Symptoms = () => {
  ...
  const [userLocation, setUserLocation] = useState("Pune, Maharashtra"); // default label
  ...
  useEffect(() => {
    ...
    navigator.geolocation.getCurrentPosition(
      (position) => {
        ...
        console.log("Got GPS coords from browser:", latitude, longitude);
        setUserCoords({ lat: latitude, lon: longitude });
        if (!userLocation || userLocation === "Pune, Maharashtra") {
          setUserLocation("Detecting location…");
        }
      },
      (error) => {
        console.error("Geolocation error:", error.code, error.message);
        setUserLocation("Enable GPS & allow location (Pune fallback)");
      },
  ...
  const refreshLocation = () => {
    ...
    navigator.geolocation.getCurrentPosition(
      (position) => {
        const { latitude, longitude } = position.coords;
        console.log("Refreshed GPS coords:", latitude, longitude);
        setUserCoords({ lat: latitude, lon: longitude });
        // backend will reverse-geocode on next message and send resolvedLocation
      },
      (error) => {
        console.error("Geolocation refresh error:", error.code, error.message);
        setUserLocation("Enable GPS & allow location (fallback)");
      },
      { enableHighAccuracy: true, timeout: 15000, maximumAge: 0 }
    );
  };
  ...
  const sendMessage = async () => {
    ...
    try {
      const payload = {
        ...
        userLocation, // label for AI (may be "Detecting location…")
        lat: userCoords?.lat || null, // GPS for reverse geocode + Overpass
        lon: userCoords?.lon || null,
      };

      console.log("Sending to /api/ai/chat:", payload);
      ...
      if (Array.isArray(response.data.nearbyClinics)) {
        setNearbyClinics(response.data.nearbyClinics);
      }

      if (response.data.resolvedLocation) {
        setUserLocation(response.data.resolvedLocation);
      }
    } catch (error) {
      console.error(
        "AI chat error:",
  ...
  const placeholderShort = "Describe your symptoms…";
  const placeholderLong =
    "Fever? Headache? Stomach pain? Get advice + nearby clinics…";
        ...
        <div className="max-w-5xl mx-auto px-3 sm:px-6 py-2.5 sm:py-3 flex items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-2">
            <span className="hidden sm:inline text-xs text-base-content/60">
              Powered by Gemini 2.5 + OpenStreetMap
            </span>
              ...
              {messages.length === 0 && (
                <div className="flex justify-center mt-4">
                  <div className="bg-base-100 border border-base-300/80 rounded-2xl px-4 py-3 text-xs sm:text-sm text-base-content/70 max-w-[90%]">
                    🩺 Describe symptoms like "I have fever and headache for 2
                    days" for instant advice + nearby clinics in{" "}
            ...
            <div className="px-3 sm:px-5 py-2.5 sm:py-4 bg-base-200/80">
              <div className="flex flex-col gap-2">
                <div className="hidden sm:flex flex-wrap gap-2 text-[11px] text-base-content/60 mb-1">
                  ...
                  {/* UPDATED: refresh location button */}
                  <button
                    type="button"
                    onClick={refreshLocation}
                    className="px-2 py-1 rounded-full bg-success/20 text-success text-xs hover:bg-success/30"
                  >
                    📍 {userLocation} (tap to refresh)
                  </button>
                ...
                <div className="flex sm:hidden justify-between items-center text-[10px] text-base-content/50 mt-0.5">
                  ...
                  <span>Free · 100% Offline Clinics</span>
                </div>
              </div>
            </div>
          </section>
